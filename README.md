...
*   **用户友好的界面**: 通过 Streamlit 提供直观的界面，方便用户上传文件和配置参数。
...
2.  **配置参数**:
    *   **上传 Excel 文件**: 在 Streamlit 界面的左侧，使用文件上传组件上传您要对比的 Excel (`.xlsx`) 文件。请确保上传至少两个文件。
    *   **输出目录**: 输入保存对比结果报告的文件夹路径。
    *   **Kimi API 密钥**: 输入您的 DashScope API 密钥。

3.  **开始对比**:
    点击“开始对比分析”按钮。脚本将处理您上传的文件，进行两两对比，并将详细的差异报告保存在输出目录中，同时生成一个总览报告。
